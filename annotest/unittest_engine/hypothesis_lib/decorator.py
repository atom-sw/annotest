...
def getAstCallForJust(value: ast.expr) -> ast.Call:
    astJust = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="just", ctx=ast.Load()
        ),
        args=[value],
        keywords=[],
    )
...
def _getAstCallForOneOf(items: List[ast.Call]) -> ast.Call:
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="one_of", ctx=ast.Load()
        ),
        args=items,
        keywords=[],
    )
...
def getAstKeywordForDataStrategy() -> ast.keyword:
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="data", ctx=ast.Load()
        ),
        args=[],
        keywords=[],
    )
...
def _getAstCallForIntegers(argumentTypeInformation: argument_type.Integers) -> ast.Call:
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="integers", ctx=ast.Load()
        ),
        args=[],
        keywords=[
            ast.keyword(
...
def _getAstCallForFloats(argumentTypeInformation: argument_type.Floats) -> ast.Call:
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="floats", ctx=ast.Load()
        ),
        args=[],
        keywords=[
            ast.keyword(
            ...
            ast.keyword(
                arg="allow_infinity",
                value=ast.Constant(
                    value=argumentTypeInformation.allow_infinity, kind=None
                ),
            ),
            ast.keyword(
                arg="width",
            ...
            ast.keyword(
                arg="exclude_min",
                value=ast.Constant(
                    value=argumentTypeInformation.exclude_min, kind=None
                ),
            ),
            ast.keyword(
                arg="exclude_max",
                value=ast.Constant(
                    value=argumentTypeInformation.exclude_max, kind=None
                ),
            ),
        ],
    )
...
def _getAstCallForSampled(argumentTypeInformation: argument_type.Sampled) -> ast.Call:
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="sampled_from", ctx=ast.Load()
        ),
...
def _getAstCallForArrayShapes(
    argumentTypeInformation: argument_type.ArrayShapes,
) -> ast.Call:
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="hynp", ctx=ast.Load()),
            attr="array_shapes",
            ctx=ast.Load(),
        ),
        args=[],
...
def _getAstCallForNpArrays(argumentTypeInformation: argument_type.NpArrays) -> ast.Call:
    ...
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="hynp", ctx=ast.Load()), attr="arrays", ctx=ast.Load()
        ),
        args=[],
        keywords=[
...
def _getAstCallForTuples(argumentTypeInformation: argument_type.Tuples) -> ast.Call:
    ...
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="tuples", ctx=ast.Load()
        ),
...
def _getAstCallForDictionaries(
    argumentTypeInformation: argument_type.Dictionaries,
) -> ast.Call:
    keysAstCall: ast.Call = _getAstCallForArgumentTypeInformation(
        argumentTypeInformation.keys
    )
    valuesAstCall: ast.Call = _getAstCallForArgumentTypeInformation(
        argumentTypeInformation.values
    )
    ...
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="st", ctx=ast.Load()), attr="dictionaries", ctx=ast.Load()
        ),
        args=[],
        keywords=[
            ast.keyword(arg="keys", value=keysAstCall),
...
def _getAstCallForMultipleTypes(
    argumentTypeInformation: argument_type.MultipleTypes,
) -> ast.Call:
    argumentTypeAstCallList = []
    for argT in argumentTypeInformation.args:
        cAstCall = _getAstCallForArgumentTypeInformation(argT)
...
def _getAstCallForIntegerLists(
    argumentTypeInformation: argument_type.IntegerLists,
) -> ast.Call:
    astCall = ast.Call(
        func=ast.Name(
            id=constant.ArgumentTypeInformation.IntegerLists.StrategyName,
            ctx=ast.Load(),
        ),
        args=[],
        keywords=[
...
def _getAstCallForArgumentTypeInformation(
    argumentTypeInformation: argument_type.ArgumentTypeInformation,
) -> ast.Call:
    astCall = None
...
def getGivenDecorator(keywords: List[ast.keyword]) -> ast.Call:
    givenDecorator = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="hy", ctx=ast.Load()), attr="given", ctx=ast.Load()
        ),
        args=[],
        keywords=keywords,
    )
...
def _getAstKeywordForHealthCheck() -> ast.keyword:
    astKeyword: ast.keyword = ast.keyword(
        arg="suppress_health_check",
        value=ast.List(
            elts=[
                ast.Attribute(
                    value=ast.Attribute(
...
def getSettingsDecorator(
    hasPrecondition: Optional[bool] = None,
    deadlineDecorator: Optional[decorator_type.DeadlineDecorator] = None,
) -> ast.Call:
    ...
    astCall = ast.Call(
        func=ast.Attribute(
            value=ast.Name(id="hy", ctx=ast.Load()), attr="settings", ctx=ast.Load()
        ),
        args=[],
        keywords=settingsAstKeywords,
    )
    return astCall
